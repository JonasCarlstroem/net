...
struct method {
  ...
  public:
    ...
    const bool equals(const method &other) const {
    ...
    static method parse(const string &method) {
        ...
        std::transform(
            formatted.begin(), formatted.end(), formatted.begin(), ::toupper
        );
    ...
    friend const std::istringstream &
    operator>>(std::istringstream &stream, method &method) {
        string str_method;
...
std::map<string, method> method::_internal = {
    {"GET", method::Get},
    {"POST", method::Post},
    {"PUT", method::Put},
